...
if not gemini_api_key or not HF_API_TOKEN:
    st.error("API keys not found. Please create a .env file with GEMINI_API_KEY and HF_API_TOKEN.")
    st.stop()
            ...
            with st.form("choice_form"):
                ...
                if submitted:
                    with st.spinner("The story continues..."):
                        ...
                        if not new_choices or any("conclusion" in choice.lower() for choice in new_choices):
                                st.session_state.game_over = True

                        st.rerun()

            if st.button("Restart Story", type="secondary"):
                # Clean up old files
                if st.session_state.narration_file and os.path.exists(st.session_state.narration_file):
                    os.remove(st.session_state.narration_file)
                if st.session_state.image_file and os.path.exists(st.session_state.image_file):
